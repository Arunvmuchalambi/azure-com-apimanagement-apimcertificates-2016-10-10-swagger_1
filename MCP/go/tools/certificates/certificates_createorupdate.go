...
func Certificates_createorupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCertificates_createorupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_certificates_certificateId",
		...
		mcp.WithString("data", mcp.Required(), mcp.Description("Input parameter: Base 64 encoded certificate using the application/x-pkcs12 representation.")),
		mcp.WithString("password", mcp.Required(), mcp.Description("Input parameter: Password for the Certificate")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Certificates_createorupdateHandler(cfg),
	}
}
